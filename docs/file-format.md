...
歴史的な誤称。実体は **SMS と +メッセージを統合したメッセージストア**で、
本文・送受信の日時と向き・メディア配信の記録がすべてここに入る。

セクションの中身も同じ 10 バイト TLV の入れ子で、`u32 の件数` から末尾まで
誤差ゼロで辿れる。

```
[u32 peerCount]
peerCount × TLV(0x0002)              ピア バケット
  ├ header (field1 = 44 + ピア ID 長)
  └ TLV(0x0003) × n → TLV(0x0004)    会話レコードと終端
```

`0x0003` の先頭 u16 が種別で、0 = 本文、4 = メディア配信、8 = サービス定義。
フィールド構成は
[section-0x0001-inbox.md](./section-0x0001-inbox.md) 参照。

コード: [src/parser/settings.ts](../src/parser/settings.ts) の `parseSettings`
...
- **anchor** — ファイル内を走査して特定パターンを探すための既知バイト列。
  SETTINGS の本文抽出では使っていたが、レコードのフィールド値を目印と
  取り違えたもので、構造を辿る実装に置き換えて廃止した
